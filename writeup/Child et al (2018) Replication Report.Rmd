...
child2018rep_data_link <- getURL("https://raw.githubusercontent.com/psych251/child2018/b8e168688bd165ae479d5c440ec6962d51921b60/data/psych251_child2018_pilotA.csv")
...
filtered_data = child2018rep_data %>%
  ...
  select(-c("StartDate", "EndDate", "Status", "IPAddress", "Progress", "Finished", "RecordedDate", "ResponseId", "RecipientLastName", "RecipientFirstName", "RecipientEmail", "ExternalReference", "LocationLatitude", "LocationLongitude", "DistributionChannel", "UserLanguage", "Duration..in.seconds.", "D2.NATIVE.ENGLISH", "D3.READING.DISORDER"),
         -starts_with("FILLER"),
...
long_ER_data %>%
  summarise(Mean.Age = mean(Age, na.rm = T),
            StdDev.Age = sd(Age, na.rm = T),
            Range.Age = paste(min(Age, na.rm = T), "-", max(Age, na.rm = T)),
            n = sum(!is.na(Age)))
...
regression_RT_data = long_filtered_RT_data %>%
  group_by(Subject) %>%
  do(model = lm (ln_RT ~ Character_Count, data = long_filtered_RT_data))

##code below needs work, gets Error "Unknown or uninitialised column: `na.action`.Unknown or uninitialised column: `residuals`."
#long_filtered_RT_data$Resid_RT <- residuals(regression_RT_data)
#then need to calculate error

####ER analysis: means and standard deviations
long_ER_data %>% 
  group_by(Perspective, Valence) %>%
  summarise(Mean_ER = mean(ER, na.rm=T),
            StdDev_ER = sd(ER, na.rm=T),
            SEM_ER = sd(ER, na.rm=T)/sqrt(length(ER[!is.na(ER)])))
```
...
```{r}
####this will be updated with log residual RTs once I figured out that step
ggplot(long_filtered_RT_data, aes(x=Valence, y=ln_RT, fill=Perspective)) +
  geom_bar(position="dodge", stat="identity") + 
  scale_fill_brewer(palette="Set1")

####add scale adjustments with final data to match original figure; needs error bar codes
...
```{r}
ggplot(long_ER_data, aes(x=Valence, y=ER, fill=Perspective)) +
  geom_bar(position="dodge", stat="identity", na.rm=T) + 
  scale_fill_brewer(palette="Set1")
...
```{r}
###in progress, to be replaced with regression residual RTs when that's fixed
RT_T_test = long_filtered_RT_data %>%
  pairwise_t_test(Valence ~ Perspective)
...
ER_T_test = long_ER_data %>%
  pairwise_t_test(Perspective ~ Valence)
...
```{r}
##still working on this as of 10/18/20; this code will need to be updated for regressed residual RTs once that is figured out

RT_model <- lmer(ln_RT ~ Perspective + Valence + Perspective*Valence + (1|Subject) + (1|Passage), long_filtered_RT_data)
