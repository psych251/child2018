...
RT_data_graph = long_filtered_RT_data %>%
  ...
  summarise(ln_RT_Resid = ln_RT_Resid,
    Mean_lnRTResid = mean(ln_RT_Resid, na.rm=T),
    SE_RT = sigma(regression_RT_data))

ggplot(RT_data_graph, aes(x=Valence, y=ln_RT_Resid, fill=Perspective)) +
...
RT_model <- lmer(ln_RT_Resid ~ Perspective + Valence + Perspective*Valence + (1|Subject) + (1|Passage), long_filtered_RT_data)
