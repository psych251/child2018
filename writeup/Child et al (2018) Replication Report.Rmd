...
1. The participant clicked on the number to rate their own emotion, rather than typing it in on their own keyboard, 
2. The experiment was implemented using Qualtrics rather than E-prime, as previously mentioned, and
3. Following Pilot B, in which a participant appeared to have used a bot to progress through the experiment as quickly as possible, a delay was added to each sentence such that each participant had to spend at least a half a second on each sentence. This was calculated as an expected minimum amount of time for the participant to view the shortest presented sentences.
...
long_RT_data = RT_data %>% 
  pivot_longer(cols=-c("Subject", "Age"),
               names_to = 'Passage',
               values_to = 'RT') %>%
  filter(Passage!="Subject.1" & Passage!="Age.1", RT != 0) %>% #RT!=0 added in Pilot B to combat RT botting
...
```{r}
##still working on this as of 11/4/20
