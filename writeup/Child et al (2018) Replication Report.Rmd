...
RT_data_graph = long_filtered_RT_data %>%
  group_by(Perspective, Valence) %>%
  summarise(ln_RT_Resid = ln_RT_Resid,
    SE_RT = sd(ln_RT_Resid = ln_RT_Resid,  na.rm=T)/sqrt(length(ln_RT_Resid[!is.na(ln_RT_Resid)])))

ggplot(long_filtered_RT_data, aes(x=Valence, y=ln_RT_Resid, fill=Perspective)) +
  geom_bar(position="dodge", stat="identity") + 
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=ln_RT_Resid-SE_RT, ymax=ER-SE_RT), width=.5, position=position_dodge(.9))
...
ggplot(ER_data_graph, aes(x=Valence, y=ER, fill=Perspective)) +
  geom_bar(position="dodge", stat="identity", na.rm=T) + 
  scale_fill_brewer(palette="Set1") +
  geom_errorbar(aes(ymin=ER-SE_ER, ymax=ER+SE_ER), width=.5, position=position_dodge(.9))
